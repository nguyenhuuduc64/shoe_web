...
export default {
  ...
  // Tìm sản phẩm trong giỏ hàng (bao gồm kiểm tra size nếu có)
  let item = cart.find(p => p.id === product.SP_ma && p.size === (this.selectedSize || "Mặc định"));

  if (item) {
    // Nếu sản phẩm đã có, tăng số lượng nhưng giới hạn tối đa 5 sản phẩm
    if (item.quantity + this.quantity > 5) {
      alert("Bạn chỉ có thể mua tối đa 5 sản phẩm!");
      item.quantity = 5; // Giới hạn số lượng
    } else {
